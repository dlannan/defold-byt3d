...
function byt3dShader:ValidateProgram( prog )
...
function byt3dShader:DumpShader( shader )
    local bufsize = 4 * 1024 * 1024 -- 4MB should be plenty
    local slen = ffi.new("int[1]")
    local buff = ffi.new("char["..bufsize.."]")
    ffi.fill(buff, bufsize, 0)

    gl.glGetShaderSource(shader, bufsize, slen, buff)
    print("Shader Source: \n", ffi.string(buff))
end
